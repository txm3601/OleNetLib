...
if ( ~param.useIttiKochInsteadOfGBVS )
  load mypath;
  ufile = sprintf('%s_m%s_%s.mat',num2str(salmapsize),num2str(param.multilevels),num2str(param.cyclic_type));
  ufile(ufile==' ') = '_';
  ufile = [ pathroot '\initcache\' ufile ];
  if ( exist(ufile) )
    grframe = load(ufile);
    grframe = grframe.grframe;
  else
    grframe = graphsalinit( salmapsize , param.multilevels , 2, 2, param.cyclic_type );
    % eval(sprintf('save -mat %s grframe',ufile));
    save(ufile,'grframe');
  end
else
  grframe = [];
